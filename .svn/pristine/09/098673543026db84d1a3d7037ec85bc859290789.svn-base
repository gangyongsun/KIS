...
import cn.com.goldwind.kis.mmseg.main.rule.LargestAvgLenRule;
import cn.com.goldwind.kis.mmseg.main.rule.LargestSumDegreeFreedomRule;
import cn.com.goldwind.kis.mmseg.main.rule.MaxMatchRule;
import cn.com.goldwind.kis.mmseg.main.rule.Rule;
import cn.com.goldwind.kis.mmseg.main.rule.SmallestVarianceRule;
...
public class ComplexSeg extends Seg {
	...
	private List<Rule> otherRules = new CopyOnWriteArrayList<Rule>();
	// 测试为true
	...
	public synchronized Chunk seg(Sentence sen) {
		...
		CopyOnWriteArrayList<Integer>[] tailLens = new CopyOnWriteArrayList[2]; // 记录词尾部允许的长度

		for (int i = 0; i < 2; i++) {
			// tailLens[i] = new ArrayList<Integer>();
			tailLens[i] = new CopyOnWriteArrayList<Integer>();
		}
			...
			for (int aIdx = tailLens[0].size() - 1; aIdx >= 0; aIdx--) {
				...
				for (int bIdx = tailLens[1].size() - 1; bIdx >= 0; bIdx--) {
					...
					if (showChunk) {
						...
						System.out.println("mm" + ck);
					}
				}
			}
			// maxLen个字符已经处理完
			sen.addOffset(maxLen);
			// ArrayList<Chunk> chunks = mmr.remainChunks();
			CopyOnWriteArrayList<Chunk> chunks = mmr.remainChunks();
	...
	private Chunk createChunk(Sentence sen, char[] chs, int[] tailLen, int[] offsets, CharNode[] cns/* , char[][] cks */) {
		...
		for (int i = 0; i < 3; i++) {
			if (offsets[i] < chs.length) {
				...
				ck.getWords()[i] = new Word(chs, sen.getStartOffset(), offsets[i], tailLen[i] + 1);
				if (tailLen[i] == 0) { // 单字的要取得"字频计算出自由度"
					CharNode cn = cns[i];
					if (cn != null) {
						ck.getWords()[i].setDegree(cn.getFreq());
					}
				}
			}
		}
