        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function (value, row, index) {//自定义显示，也可以写标签
                	operate=
                	'<shiro:hasPermission name="resource/grantresource2role">'+
					'<a class="btn btn-outline btn-info btn-sm" onclick="javascript:selectResourceById('+row.id+');"><span class="fa fa-filter"></span> 选择权限</a>'+
					'</shiro:hasPermission>'
                    return operate;
                }
            }
