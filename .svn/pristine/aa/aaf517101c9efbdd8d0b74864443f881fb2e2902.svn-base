        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function (value, row, index) {//自定义显示，也可以写标签
                	operate=
                	'<shiro:hasPermission name="role/grantroles2user">'+
					'<a class="btn btn-outline btn-info btn-sm" onclick="javascript:selectRoleById('+row.id+');"><span class="fa fa-filter"></span> 选择角色</a>'+
					'</shiro:hasPermission>'
                    return operate;
                }
            }
