		...
		},
		{
            ...
            formatter: function (value, row, index) {//自定义显示，也可以写标签
            	var accept=
					...
					'<a class="btn btn-outline btn-info btn-sm" onclick="console.log(\''+row.content+'\')"><span class="fa fa-save"></span> 采纳</a>'+
					'</shiro:hasPermission>&nbsp;';
            	var refuse=
            		'<shiro:hasPermission name="user/deactive">'+
            		'<a class="btn btn-outline btn-info btn-sm" onclick="console.log(\''+row.content+'\')"><span class="fa fa-save"></span> 拒绝</a>'+
            		'</shiro:hasPermission>';
            	var returnValue=accept+refuse;
                return returnValue;
            }
		}]
	});
