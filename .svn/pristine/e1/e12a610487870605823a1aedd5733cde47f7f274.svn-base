...
/**
 * 源码来源： http://blog.chenlb.com/category/mmseg4j 词典类. 词库目录单例模式.<br/>
 * 保存单字与其频率,还有词库.<br/>
...
public class Dictionary {
	...
	private Map<Character, CharNode> loadDic(File wordsPath) throws IOException {
		...
		if (charsFile.exists()) {
			...
			charsIn = this.getClass().getResourceAsStream("/data_entity/chars.dic");
			charsFile = new File(this.getClass().getResource("/data_entity/chars.dic").getFile()); // only for log
		}
		...
		InputStream wordsDicIn = this.getClass().getResourceAsStream("/data_entity/words.dic");
		if (wordsDicIn != null) {
			File wordsDic = new File(this.getClass().getResource("/data_entity/words.dic").getFile());
	...
	// public ArrayList<Integer> maxMatch(CharNode node, ArrayList<Integer>
	// tailLens, char[] sen, int offset) {
	public CopyOnWriteArrayList<Integer> maxMatch(CharNode node, CopyOnWriteArrayList<Integer> tailLens, char[] sen, int offset) {
		tailLens.clear();
		tailLens.add(0);
		if (node != null) {
...
//	}
	...
	public static File getDefalutPath() {
		if (defalutPath == null) {
			...
			boolean flag=defalutPath.exists();
			if (!flag) {
				defalutPath = new File(PropertiesUtil.getValueByKey("SERVER_DICT_PATH", "config.properties"));
			}
