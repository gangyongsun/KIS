...
public class MMSeg {
	...
	/**
	 * Adolf Frank 20161021 将私有变量移到函数内部
	 */
	// 移到函数内部
	// private Sentence currentSentence;
	// private Queue<Word> bufWord; // word 缓存, 因为有 chunk 分析三个以上.
	// private Queue<Word> bufWord = new LinkedBlockingQueue<>(); 也可以为该
	private ConcurrentLinkedQueue<Word> bufWord;
	...
	public void reset(Reader input) {
		...
		// bufWord = new LinkedList<Word>();ConcurrentLinkedQueue
		bufWord = new ConcurrentLinkedQueue<Word>();
	...
	private int readNext() throws IOException {
		...
		if (d > -1) {
			readedIdx++;
			// 20150119 Frank suny change character to lower case
		...
		if (word == null) {
			...
			while (read && (data = readNext()) != -1) {
				...
				case Character.MODIFIER_LETTER:
					/*
					 * 1. 0x410-0x44f -> А-я //俄文 2. 0x391-0x3a9 -> Α-Ω //希腊大写 3. 0x3b1-0x3c9 -> α-ω
					 * //希腊小写
					 */
					data = toAscii(data);
					NationLetter natLeter = getNation(data);
					if (natLeter == NationLetter.UNKNOW) {
						read = true;
						break;
					}
				...
				case Character.OTHER_LETTER:
					/*
					 * 1. 0x3041-0x30f6 -> ぁ-ヶ //日文(平|片)假名 2. 0x3105-0x3129 -> ㄅ-ㄩ //注意符号
					 */
					bufSentence.appendCodePoint(data);
				...
				case Character.OTHER_NUMBER:
					...
					break;
				// 20140724 suny
				case Character.OTHER_PUNCTUATION: // 24 一般符号。.，,!！@#&*%
				case Character.INITIAL_QUOTE_PUNCTUATION: // 29 中文左引号,全半角
				case Character.FINAL_QUOTE_PUNCTUATION: // 30 中文右引号,全半角
				case Character.END_PUNCTUATION: // 22 }]) 全半角
				case Character.START_PUNCTUATION: // 21 {[( 全半角
					// case Character.CONNECTOR_PUNCTUATION: // 23
				case Character.DASH_PUNCTUATION: // 20 -
				case Character.CURRENCY_SYMBOL: // 26 $
				case Character.MATH_SYMBOL: // 25 | +=
					bufSentence.appendCodePoint(data);
					wordType = Word.TYPE_PUNCT;
					readChars(bufSentence, new ReadCharByType(Character.OTHER_PUNCTUATION));
					...
					break;
				/*
				 * 不进行切分,丢失不处理的符号或内容 default : //其它认为无效字符 read = true;
				 */
				default:
					bufSentence.appendCodePoint(data);
					readChars(bufSentence, new ReadCharByType(Character.START_PUNCTUATION));
			...
			if (currentSentence != null) {
				do {
					...
					// System.out.println(Thread.currentThread().getName() + "当前的串\t" +
					// currentSentence.toString());
					// System.out.println(Thread.currentThread().getName() + "当前的chunk\t" +
					// chunk.toString());
